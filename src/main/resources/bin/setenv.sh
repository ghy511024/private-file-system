...
JAVA_OPTS=" $JAVA_OPTS $JAVA_MONITOR_OPTS -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Dfile.encoding=UTF-8 -DAppBaseDir=$APP_BASE"

if [[ $WCloud_Env != "Product" ]]; then
    JAVA_OPTS=" $JAVA_OPTS -Xrunjdwp:transport=dt_socket,address=18848,server=y,suspend=n -Xdebug -Xnoagent -Djava.compiler=NONE"
fi
